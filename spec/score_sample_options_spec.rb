...
describe ScoreSampleOptions do
  ...
  describe "defaults" do
    subject { ScoreSampleOptions.new(default_options.merge(:exclude_votes_by => 42)) }
  ...
  it "handles a string for limit" do
    ScoreSampleOptions.new(default_options.merge(:limit => '13')).limit.should eq(13)
  end

  it "deprecates :shuffle in favor of randomize" do
    segment = ScoreSampleOptions.new(default_options.merge(:shuffle => true, :randomize => false))
  ...
  describe "invalid parameters" do
    ...
    it "complains about missing limit" do
      ->{ ScoreSampleOptions.new(minimal_options.merge(:limit => nil)) }.should raise_error(ArgumentError)
    end

    it "complains about missing segments" do
      ->{ ScoreSampleOptions.new(minimal_options.merge(:segments => nil)) }.should raise_error(ArgumentError)
    end

    it "complains about empty segments" do
      ->{ ScoreSampleOptions.new(minimal_options.merge(:segments => [])) }.should raise_error(ArgumentError)
    end

    it "complains about invalid segments" do
      invalid_segment = {:field => 'invalid_filter'}
      ->{ ScoreSampleOptions.new(minimal_options.merge(:segments => [invalid_segment])) }.should raise_error(ArgumentError)
    end
  end
  ...
  describe "identity" do
    ...
    it "excludes votes" do
      segment = ScoreSampleOptions.new(default_options.merge(:exclude_votes_by => 42))
      segment.exclude_votes_by.should eq(42)
    end

    it "can't exclude votes without an identity id" do
      segment = ScoreSampleOptions.new(default_options.merge(:exclude_votes_by => nil))
      segment.exclude_votes_by.should be_nil
    end

    it "ignores deprecated fields" do
      segment = ScoreSampleOptions.new(default_options.merge(:identity_id => 1337, :include_own => false, :exclude_votes_by => nil))
      segment.exclude_votes_by.should be_nil
    end

    context "deprecated" do
      it "excludes votes" do
        segment = ScoreSampleOptions.new(default_options.merge(:identity_id => 1337, :include_own => false))
        segment.exclude_votes_by.should eq(1337)
      end

      it "doesn't exclude votes" do
        segment = ScoreSampleOptions.new(default_options.merge(:identity_id => 1337, :include_own => true))
        segment.exclude_votes_by.should be_nil
      end
    end
  end
